...
export function DrawingCanvas({ chart, series, candles, symbol, revision }: DrawingCanvasProps) {
  ...
  const onPointerDown = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (!needsTwo) {
      add(symbol, { type: tool as Drawing['type'], points: [anchor], color, width });
      setPending([]);
      return;
    }
    ...
    if (pending.length === 0) {
      ...
      if (tool !== 'measure') {
        add(symbol, { type: tool as Drawing['type'], points: [pending[0], anchor], color, width });
      }
      setPending([]);
      setCursor(null);
    }
  ...
  return (
    <canvas
      ...
      style={{
        // Let chart panning through when no tool is armed and nothing is grabbable.
        pointerEvents: active || drawings.length > 0 ? 'auto' : 'none',
